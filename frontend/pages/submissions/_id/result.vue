...
<!-- DUPE a copy of this View when finalised for the /submissions/verify for use in community voting on challenges -->
<template>
  <v-stepper v-model="e1" vertical>
    ...
    <v-stepper-content step="1">
      <v-form ref="form" class="form-page" v-model="valid" lazy-validation>
        ...
        <p>[ ISSUE REPORTED BOX ]</p>
        <p>{{radioissue}}</p>
        <v-radio-group v-model="radioissue">
          <div slot="label">
            Is this an
            <strong>Integrity issue</strong>? Reply with
          </div>
          <v-radio value="no">
            <div slot="l
            abel">
              No, we do not deem the report to be a
              <strong class="success--text">safe</strong>
            </div>
          </v-radio>
          <v-radio value="yes">
            <div slot="label">
              Yes, thankyou for reporting. We are awarding the
              <strong
                class="error--text"
              >issue bounty</strong>
            </div>
    ...
    <v-stepper-content step="2">
      ...
      <v-btn color="pink" @click="e1 = 3">RELESE BOUNTY &amp; JACKPOT</v-btn>
    </v-stepper-content>
    ...
    <v-stepper-content step="3">
      <v-card class="mb-5" color="grey lighten-1" height="200px">
        <p>
          We just awarded a 500EOS bounty plus 325 Jackpot.
