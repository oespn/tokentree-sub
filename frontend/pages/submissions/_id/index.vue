<template>
  <v-layout column>
    <v-toolbar dark>
      <v-btn icon nuxt-link to="/submissions">
    ...
    <v-list two-line>
      <v-list-tile
        ...
        :key="index"
        nuxt-link
        :to="menu.action && menu.action.to"
        :href="menu.action && menu.action.href"
      >
        <v-list-tile-action>
...
export default {
  computed: {
    ...
    menus() {
      const sum = this.report.verifications.reduce(
        (prev, { health }) => prev + health,
        0
      );
      ...
      return [
        ...
        {
          icon: "event",
          title: this.report.plantedAt,
          subtitle: "Reported date"
        },
        ...
        {
          icon: "verified_user",
          title: `${this.report.verifications.length} verification(s)`,
          subtitle: "Number of verifications",
          action: {
            icon: "chevron_right",
            to: `/submissions/${this.report.id}/verifications`
          }
        }
      ];
    }
