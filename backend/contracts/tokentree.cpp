...
class treestory : public eosio::contract {
  public:
    ...
    /// @abi action
    void addoffer(account_name username, double geo, uint32_t radius, const std::string& description, uint64_t offervalue, uint64_t days) {
        require_auth(username); // this user is a sponsor
        // Let's make sure the primary key doesn't exist

        const uint64_t usecsperday = 24*3600*1000000;
        auto now = current_time();
        auto expires = now + (usecsperday * days);
        ...
        action(
            permission_level{ username, N(active) },
            N(eosio.token), N(transfer),
            std::make_tuple(username, _self, offervalue) //std::string("TRE")
         ).send();
    ...
    /// @abi action
    void plant(account_name username, double geo, double treepatchkey) {
    ...
    /// @abi action
    void validate(account_name username, double treepatchkey, std::string comment, uint8_t health) {
    ...
    /// @abi action
    void payout(account_name username, double treepatchkey) {
        require_auth(username); // this user is a validator
        ...
        auto treepatch = _treepatchs.find(treepatchkey);
        eosio_assert(!treepatch->closed, "You cannot payout a closed offer");
        print("asset_passed");

        auto closed = current_time();
        print(closed);
        print("Got time");
        ...
        std::vector<double> treesForModify;
          ...
          {
            auto tree = _trees.find(key);
              //seek the planter & payout
            auto toplanter = tree->planter;
            action(
                permission_level{ _self, N(active) },
                N(eosio.token), N(transfer),
                std::make_tuple(_self, toplanter, planterpay) //, std::string("TRE"))
            ).send();
            print("toplanter");
          }
        ...
        auto verifierpay = verifiertokens / treepatch->validatecount;
          ...
          // update the paid out tokens to each validator
          for (double key : keysForModify)
          {
            auto validator = _validations.find(key);
            print(N(validator));
            auto to = validator->validator;
            //seek the validator & payout
            action(
                permission_level{ _self, N(active) },
                N(eosio.token), N(transfer),
                std::make_tuple(_self, to, verifierpay)//, std::string("TRE"))
            ).send();
            print("validator");
            print(to);
          }*/
  ...
  private: 
    ...
    struct treepatch {
      ...
      uint64_t offered; // set when user plants the tree
      uint64_t expires;
      uint64_t closed;

      uint16_t treecount = 0; // incremented by trees
      uint16_t validatecount = 0; // incremented by validation
    ...
    struct tree {
      ...
      uint64_t planted; // set when user plants the tree
